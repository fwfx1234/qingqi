...
>
> 当前补充说明（2026-05-31）：P0–P8 已完成。本文保留为**执行复盘 + 后续低优先级清理参考**；若与当前代码状态不一致，以主导文档和实际代码为准，先修文档再行动。
...
如果阶段开始前 `cargo check` 已失败，记录基线错误，不要把它当成本阶段引入的错误。
...
只有在架构阶段全部落地后，才集中执行测试收口：

```powershell
cargo test --workspace
```

若 `cargo test --workspace` 在当前 Windows 环境上因 pagefile / 编译器并发资源波动失败，先切换为 `cargo test --workspace -j 1` 或定向包测试，不要把资源级级联错误直接判定为架构问题。
...
> 当前进度：P0–P8 已完成，`cargo fmt --all`、`cargo check -p qingqi-app`、`cargo check -p qingqi`、`cargo check --workspace`、`cargo test --workspace -j 1 --quiet` 已通过。当前重点是收尾文档与剩余低优先级清理。
...
已完成：
- 根 `#![allow(dead_code)]` 已删除
- `view_model.rs` 已删除
- `core/mod.rs` 中 `pub mod view_model;` 已移除
...
已完成：`Plugin::database_specs()` 与各插件实现已删除，DB 声明保留 descriptor 路线。
...
已完成：`ThemeAccent` 与 `accent_to_theme` 已移除，现统一使用 `PluginAccent`。
...
已完成：`app/events.rs` 已删除，调用点已改为最终归属。
...
已完成：
- 根 `Cargo.toml` 已改为 workspace
- `src/`、`assets/`、`build.rs`、`icon_raster.rs` 已移入 `crates/qingqi/`
- `cargo check -p qingqi` 已通过
...
已完成：
- 已新增 `crates/qingqi-plugin`
- `core` 中 SDK 类型已抽到 `qingqi-plugin`
- 现有宿主代码已切到统一 SDK 类型来源
- `cargo check -p qingqi-plugin`、`cargo check --workspace` 已通过
...
已完成：
- 已新增 `crates/qingqi-ui`
- `theme` / `theme_mode` / `ui` / `text_input` / `assets` 已迁入 `qingqi-ui`
- `platform/svg_icon.rs` 已改为接收已解析路径，资源解析留在 `qingqi-ui::assets`
- `crates/qingqi/src/features/*` 与 app 侧 UI 引用已切换为 `qingqi_ui::*`
- 旧 `app/ui`、`app/theme*`、`app/text_input`、`app/assets` 副本源码已删除
...
已完成：
- 已新增 `crates/qingqi-platform`
- `apps` / `clipboard` / `display` / `hotkey` / `low_level_hook` / `macos` / `power` / `shell` / `svg_icon` / `tray` 已迁入 `qingqi-platform`
- `tray` 与 `svg_icon` 已改为平台自洽实现，不依赖 `qingqi-ui`
- `crates/qingqi/src` 中平台调用已整体切换为 `qingqi_platform::*`
- `crates/qingqi/src/platform/` 旧目录已删除
...
已完成：
- 已新增 `crates/qingqi-core`
- `PluginManager` / `FeatureRegistry` / `PluginDescriptor` / `BuildCx` / `CommandUsageStore` 已迁入 `qingqi-core`
- `crates/qingqi/src` 中宿主实现引用已切换到 `qingqi_core::*`
- `crates/qingqi/src/core/plugin.rs` / `registry.rs` / `command_usage.rs` 旧实现已删除
- `shortcut` / `keymap` 仍留在当前 crate，待后续按文档进入 `qingqi-app`
...
当前进展：
- 已新增并接入首批 feature crates：
  - `qingqi-feature-about`
  - `qingqi-feature-anti-peeping`
  - `qingqi-feature-gpui-demo`
  - `qingqi-feature-json-parser`
  - `qingqi-feature-qr-code`
- 以上 5 个插件已从 `crates/qingqi/src/features/` 移除，bin 侧注册已改为依赖新 crate
- 已新增并接入第二批 feature crates：
  - `qingqi-feature-api-debugger`
  - `qingqi-feature-clipboard`
  - `qingqi-feature-download-manager`
  - `qingqi-feature-ftp-sftp-ssh-client`
  ...
  - `qingqi-feature-quick-launch`
  - `qingqi-feature-system-settings`
- 上述 6 个插件已补齐统一 crate 入口（`databases()` / `build(...)`），bin 侧注册已改为依赖新 crate
- `crates/qingqi/src/features/` 中旧目录已移除：
  - `api_debugger`
  - `clipboard`
  - `download_manager`
  - `ftp_sftp_ssh_client`
  ...
  - `quick_launch`
  - `system_settings`
- `src/features/stub_plugin.rs` 已确认零引用并删除
- `ClipboardContext` 已进入 `qingqi-plugin`，app 侧已改为持有 `Arc<dyn ClipboardContext>`，不再直接依赖 `ClipboardService`
- `system_settings` 已切到句柄注入形态（theme/app-index/shortcut host handles），为后续 `qingqi-app` 收口准备完成
- 所有 feature crate 已迁完，`crates/qingqi/src/features/` 当前只保留 bin 侧注册壳 `mod.rs` / `registry.rs`
...
已完成：
- 已新增 `crates/qingqi-app`
- `runtime` / `window_controller` / `launcher` / `app_index` / `app_index_store` / `app_catalog` / `background` / shortcut 服务已迁入 `qingqi-app`
- `qingqi-app` 已提供 `bootstrap() -> Result<AppHost>` 与 `run(host, clipboard)` 终态 API
- `qingqi-app` 内已通过 adapter 方式提供 theme / app-index / shortcut host handle，未引入 feature 反向依赖
- `cargo check -p qingqi-app` 已通过
...
已完成：
- `crates/qingqi/src/main.rs` 已收敛为 `bootstrap -> register_builtin_plugins -> run`
- 内置插件注册已收敛到 `crates/qingqi/src/features/registry.rs`
- `qingqi` 已依赖 `qingqi-app` 作为唯一 app 外壳入口
- `cargo check -p qingqi` 已通过
...
当前验收状态：
- 已通过：`cargo fmt --all`
- 已通过：`cargo check -p qingqi-app`
- 已通过：`cargo check -p qingqi`
- 已通过：`cargo check --workspace`
- 已通过：`cargo tree -p qingqi-app | rg "qingqi-feature"`（无输出）
- 已通过：`cargo tree -p qingqi-core | rg "qingqi-feature"`（无输出）
- 已通过：`cargo tree -p qingqi-plugin | rg "qingqi-(core|app|platform|feature)"`（无输出）
- 已通过：定向 `cargo test -p qingqi-plugin/qingqi-core/qingqi-app -j 1`
- 已通过：`cargo test --workspace -j 1 --quiet`
- 已知环境特征：不带 `-j 1` 的并行全量测试在当前 Windows 机器上仍可能受 pagefile 资源波动影响
