...
当前拆分前代码仍是四层目录；拆分目标以 [workspace-split-guide.md](workspace-split-guide.md) 为准。无论拆分前后，依赖都必须**严格单向**，禁止反向或环：
...
- **编辑器类输入**（脚本 / JSON / 日志 / 大文本）→ `gpui_component::input::InputState`（code 模式）。
  普通单行/简单字段可用项目 `app::text_input::TextInput` adapter。**禁止给普通字段套重型 code editor。**
...
1. 配置组件自带样式参数；
2. 包本地 adapter 验证效果；
3. 两个及以上插件复用同一改造后，抽到项目公共 wrapper（拆分前为 `app::ui`/`app::ui::components`，拆分后为 `qingqi-ui`）。
...
```text
palette   (拆分前 app/theme.rs；拆分后 qingqi-ui::theme)  原始色阶 slate_*/blue_*/violet_* + 间距 space_*。【内部层，feature 禁用】
语义 token (拆分前 app/ui.rs；拆分后 qingqi-ui::ui)        bg_canvas/bg_surface/text_primary/border_light/accent_color/status_color…
组件原语   (拆分前 app/ui.rs；拆分后 qingqi-ui::ui)        section_card/row_card/stat_card/ui_button/ui_card/ui_badge/ui_chip…
```
...
- 资源定位拆分前统一走 `app::assets`（`resolve`/`resolve_string`/`ProjectAssets`），拆分后走 `qingqi-ui::assets`，
