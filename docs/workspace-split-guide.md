...
> 用法：每次开工读 §10 进度表 → 对照 [gpt-5.4-workspace-split-execution-plan.md](gpt-5.4-workspace-split-execution-plan.md) 选下一个未完成阶段 → 跑 §附录体检 + `cargo test` → 勾选 → 提交。
...
- [ ] **P0 预清理（§8）**：删全局 `#![allow(dead_code)]`、删已确认全死模块、删 `app::events` 壳。先瘦身再拆，避免垃圾进 SDK。
- [ ] **P1 workspace 骨架**：根 `Cargo.toml` 加 `[workspace]`，现有代码整体移入 `crates/qingqi/`（bin）。验证 `cargo build`（含 build.rs/assets 路径）。
- [ ] **P2 `qingqi-plugin`（SDK）**：从 `core/` 抽出 §4 列举的 trait/类型/存储/事件 → 新 crate。`crate::core::X` → `qingqi_plugin::X`。**同时**把 shortcut **声明类型**留这里、服务留 app（C1/C2 的类型侧）。验证 `cargo check -p qingqi-plugin` 且其依赖图不含上层。
- [ ] **P3 `qingqi-ui`**：theme/theme_mode/ui(+components)/text_input/assets → 新 crate；合并 `ThemeAccent` 入 `PluginAccent`（去重）；`svg_icon` 改收绝对路径（破 C3）。
- [ ] **P4 `qingqi-platform`**：OS 服务 → 新 crate（依赖最小）。
- [ ] **P5 `qingqi-core`（宿主）**：`PluginManager`/registry/usage/排序 → 新 crate，仅依赖 `qingqi-plugin`。**校验 I1**：`cargo tree -p qingqi-core` 无 features。
- [ ] **P6 `qingqi-feature-*`**：内置插件 → 每插件一个 crate，依赖 plugin/ui/platform；shortcut 服务的 `ShortcutService`/`dispatch_target` 落 app；`ClipboardContext` 由 clipboard 插件实现（破 C4 剪贴板侧）。
- [ ] **P7 `qingqi-app`**：runtime/window/launcher/app_index/background/shortcut 服务 → 新 crate。`run()` 改为接收已装配依赖（破 C4 组合根侧）。
- [ ] **P8 `qingqi`（bin）收尾**：仅留 `main` + 组合根 `register_builtin_plugins`（调用 features）+ build.rs/assets/bundle。`cargo test --workspace`，跑通启动器与各插件。
...
- 测试随各 crate 走，`cargo test --workspace` 跑全量。
...
- [ ] 移除根 `#![allow(dead_code)]`（`main.rs:1`），它压制了 **245 条**告警。拆分后各 crate 默认暴露死代码告警；个别保留处就地 `#[allow]` + 理由。
- [ ] **`core/view_model.rs`（`ViewModel/Field/FieldKind`）**：当前被 trait 式视图取代、零引用。**但**在第 4 条语境下，它可能是「声明式/进程外插件协议」的雏形 → **属 §9-D1 决策**：若确定只做进程内插件 → 删(A)；若要声明式通道 → 保留并纳入 SDK 设计。
- [ ] **`core/dict_store.rs`（`PluginDictStore`，229 行）**：插件通用 KV 存储，生产零引用（插件各用自带 store）。作为 **SDK 面向插件的统一存储**，这正是 (B)「设计在、没推广」——**建议保留进 `qingqi-plugin` 并在文档登记**「插件持久化应收敛到此」，而非删除。（与第 4 条一致：给第三方插件一个标准存储。）
- [ ] **`Plugin::database_specs()` vs `PluginDescriptor::with_databases`** 双机制：lint 证实 trait 方法从未被调用 → 删 `database_specs()`，DB 声明**只保留一种**（descriptor/manifest）。这是第 2 条「无双机制」。
- [ ] **`PluginAccent`（plugin_spec） vs `ThemeAccent`（theme）** 字段相同的两枚举 + `accent_to_theme` 桥接：合并为 SDK 的单一 `PluginAccent`，`qingqi-ui` 直接配色。
- [ ] `theme.rs` 被语义 token 取代的裸色板 + `token(name)` legacy 查找（含死 `_dark` 绑定）→ 删；`ui` 多代重复组件（`ui::badge`/`ui_badge`/`components::*`）→ 定 `components/*` 为终态、迁移引用、删旧。
- [ ] （相关，非必需）`window_controller`/`app_index` 内联 poison 恢复重复约 25/12 次 → 统一用 SDK 的 `lock_or_recover`（(B) 设计在没遵循）。
...
阶段 P0 · 预清理（§8）
- [ ] 删 `#![allow(dead_code)]`，记录告警基线
- [ ] 按 D1 处理 `view_model`；按 D3 收编 `dict_store`/存储
- [ ] 删 `database_specs()` 双机制、合并 accent 枚举、清 theme 裸色板/重复组件
- [ ] 删 `app/events.rs` 壳，引用改 `qingqi_plugin`(事件)

阶段 P1–P8 · 拆包（§6，自底向上，每步 `cargo test` + §附录体检）
- [ ] P1 workspace 骨架（bin 单成员）
- [ ] P2 qingqi-plugin（SDK）+ shortcut 类型侧
- [ ] P3 qingqi-ui（含 accent 合并、svg_icon 收绝对路径）
- [ ] P4 qingqi-platform
- [ ] P5 qingqi-core（宿主）→ 校验 I1
- [ ] P6 qingqi-feature-*（每插件一个 crate，+ ClipboardContext 实现）
- [ ] P7 qingqi-app（+ shortcut 服务、run() 接收装配）
- [ ] P8 qingqi(bin) 组合根收尾 → `cargo test --workspace`
...
```bash
rg -n "\bapp::|\bplatform::|\bfeatures::" src/core      # P5 后应空
rg -n "\bapp::|\bfeatures::"            src/platform     # 应空（core/plugin 允许）
rg -n "\bfeatures::"                    src/app          # 应空（I1）
rg -n "crate::app::\w+"                 src/features | sort | uniq -c   # 破环后仅剩将迁往 ui 的
# 拆包后：
cargo tree -p qingqi-core | rg -i "qingqi-feature"   # 应空（I1）
cargo tree -p qingqi-plugin                   # 不应出现 core/app/platform/features（I2）
```
> 注意：分组 `use crate::{ app::…, platform::… }` 跨行，`rg "use crate::app"` 会漏；用裸路径段 `\bapp::` 才扫得全。
...
fn main() -> anyhow::Result<()> {
    let host = qingqi_app::bootstrap()?;                  // db/events/paths/app_index/theme_store + 空 PluginManager
    let clipboard = qingqi_features::register_builtin(&mut host.plugins, &host.build_cx)?; // 返回 Arc<dyn ClipboardContext>
    qingqi_app::run(host, clipboard)                      // app 仅依赖 qingqi_plugin 的 trait
}
```
