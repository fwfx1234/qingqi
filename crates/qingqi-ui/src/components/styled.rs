...
use gpui::{
    App, Div, Edges, FocusHandle, Hsla, ParentElement, Pixels, Stateful, Styled,
...
impl Size {
    ...
    pub fn smaller(&self) -> Self {
        match self {
            Size::XSmall => Size::XSmall, Size::Small => Size::XSmall,
    ...
    pub fn larger(&self) -> Self {
        match self {
            Size::XSmall => Size::Small, Size::Small => Size::Medium,
            Size::Medium => Size::Large, Size::Large => Size::Large,
            Size::Size(p) => Size::Size((*p) * 1.2),
        }
    }
    pub fn input_px(&self) -> Pixels {
        match self { Size::Large => px(16.), Size::Medium => px(12.), Size::Small => px(8.), Size::XSmall => px(4.), Size::Size(p) => *p }
    }
    pub fn input_py(&self) -> Pixels {
        match self { Size::Large => px(10.), Size::Medium => px(8.), Size::Small => px(2.), Size::XSmall => px(0.), Size::Size(p) => *p }
    }
}

impl From<Pixels> for Size {
    fn from(size: Pixels) -> Self { Size::Size(size) }
}
...
impl<E: Styled> StyleSized<E> for E {
    fn button_text_size(self, size: Size) -> Self {
        match size { Size::XSmall => self.text_xs(), Size::Small => self.text_sm(), _ => self.text_size(px(14.0)) }
    }
}
...
pub(crate) fn focus_ring(
    ...
    el.child(
        div()
            ...
            .border(ring_bw)
            .border_color(Hsla { a: 0.5, ..border_focus })
    )
}

/// Extension trait adding `refine_style`, `h_flex`, `v_flex` etc to any `Styled` type.
pub trait StyledExt: Styled + Sized {
    fn refine_style(self, _style: &StyleRefinement) -> Self {
        // StyleRefinement copying is done inside the refineable macro (vendor-side);
        // this is a best-effort no-op stub that the local components can still call.
        self
    }
    #[inline(always)]
