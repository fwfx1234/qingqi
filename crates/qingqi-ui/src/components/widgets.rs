...
use gpui::{
    prelude::*, percentage, relative,
    AnyElement, App, Div, IntoElement, ParentElement, Pixels,
    RenderOnce, SharedString, StatefulInteractiveElement, Styled, StyleRefinement,
    Window, div, px,
};
...
pub enum TagVariant { #[default] Primary, Info, Success, Warning, Danger, Secondary }

impl Tag {
    pub fn new() -> Self { Self { label: None, variant: TagVariant::Primary, size: Size::Medium } }
    pub fn primary() -> Self { Self::new().with_variant(TagVariant::Primary) }
    pub fn info() -> Self { Self::new().with_variant(TagVariant::Info) }
    pub fn success() -> Self { Self::new().with_variant(TagVariant::Success) }
    pub fn warning() -> Self { Self::new().with_variant(TagVariant::Warning) }
    pub fn danger() -> Self { Self::new().with_variant(TagVariant::Danger) }
    pub fn secondary() -> Self { Self::new().with_variant(TagVariant::Secondary) }
    fn with_variant(mut self, v: TagVariant) -> Self { self.variant = v; self }
    pub fn small(mut self) -> Self { self.size = Size::Small; self }
    pub fn child(mut self, s: impl Into<SharedString>) -> Self { self.label = Some(s.into()); self }
}
...
impl RenderOnce for Tag {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let h = if self.size == Size::Small { px(20.) } else { px(24.) };
        div()
            .flex().items_center().gap_1()
            .h(h).px_2().rounded(px(4.))
            .bg(bg).text_color(fg)
            .text_size(px(11.))
            .children(self.label)
    }
...
impl Badge {
    pub fn new() -> Self { Self { count: None, dot: false, child: None } }
    pub fn count(mut self, n: usize) -> Self { self.count = Some(n); self }
    pub fn dot(mut self) -> Self { self.dot = true; self }
    pub fn child(mut self, el: impl IntoElement) -> Self { self.child = Some(el.into_any_element()); self }
}
...
impl RenderOnce for Badge {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        div()
            ...
            .when(self.dot, |d| d.child(
                div().absolute().top_0().right_0()
                    .w(px(6.)).h(px(6.)).rounded(px(3.))
                    .bg(cx.theme().danger())
            ))
            .when_some(self.count, |d, n| d.child(
                div().absolute().right(px(0.)).top(px(0.))
                    .min_w(px(16.)).h(px(16.)).rounded(px(8.))
                    .bg(cx.theme().danger()).text_color(gpui::white())
...
impl Progress {
    pub fn new() -> Self { Self { value: 0.0, bg: None } }
    pub fn value(mut self, v: f32) -> Self { self.value = v.clamp(0.0, 100.0); self }
    pub fn bg(mut self, c: impl Into<gpui::Hsla>) -> Self { self.bg = Some(c.into()); self }
    pub fn h(mut self, h: Pixels) -> Self { self }
}
...
impl DropdownMenu {
    ...
    pub fn popup_item(mut self, item: impl Into<crate::layer::context_menu::PopupMenuItem>) -> Self {
...
impl Slider {
    pub fn new() -> Self { Self { value: 0.0, min: 0.0, max: 100.0 } }
    ...
    pub fn horizontal(mut self) -> Self { self }
}
...
pub trait SelectItem: Clone {
    ...
    fn display_title(&self) -> Option<gpui::AnyElement> { None }
...
impl std::default::Default for SliderState {
    fn default() -> Self {
        Self { value: 0.0, min: 0.0, max: 100.0, step: 1.0 }
    }
}
