...
use gpui::{
    ...
    SharedString, StyleRefinement, Styled, Window, div, px, prelude::*,
};
...
impl Side {
    pub fn is_left(&self) -> bool { matches!(self, Side::Left) }
}
...
impl SidebarMenu {
    ...
    pub fn children(mut self, children: impl IntoIterator<Item = impl Into<SidebarMenuItem>>) -> Self {
...
impl Collapsible for SidebarMenu {
    ...
    fn collapsed(mut self, collapsed: bool) -> Self { self.collapsed = collapsed; self }
}
...
impl RenderOnce for SidebarMenu {
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
        div().flex().flex_col().gap_2().refine_style(&self.style).children(
            self.items.into_iter().enumerate().map(|(ix, item)| {
                div().id(ElementId::Integer(ix as u64)).child(item.collapsed(self.collapsed))
            }),
        )
    }
}
...
impl SidebarMenuItem {
    ...
    fn id(mut self, id: impl Into<ElementId>) -> Self { self.id = id.into(); self }
}

impl From<&'static str> for SidebarMenuItem {
    fn from(s: &'static str) -> Self { Self::new(s) }
}
...
impl RenderOnce for SidebarMenuItem {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        div()
            ...
            .px_3()
            .text_size(px(13.))
            .when(is_active, |d| d.bg(cx.theme().sidebar_accent()).text_color(cx.theme().sidebar_accent_foreground()))
            ...
            .when(!self.disabled, |d| {
                d.cursor_pointer().on_click(move |ev, window, cx| (self.handler)(ev, window, cx))
            })
    }
}
...
impl Sidebar {
    pub fn new(side: Side) -> Self {
        Self {
            style: StyleRefinement::default(), content: vec![], header: None,
            side, collapsible: true, collapsed: false,
        }
    }
    pub fn left() -> Self { Self::new(Side::Left) }
    pub fn right() -> Self { Self::new(Side::Right) }
    pub fn collapsible(mut self, collapsible: bool) -> Self { self.collapsible = collapsible; self }
    pub fn collapsed(mut self, collapsed: bool) -> Self { self.collapsed = collapsed; self }
    pub fn header(mut self, header: impl IntoElement) -> Self {
        self.header = Some(header.into_any_element());
        self
    }
    pub fn child(mut self, child: SidebarMenu) -> Self { self.content.push(child); self }
    ...
    fn render(mut self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        div()
            ...
            .when_some(self.header.take(), |d, header| {
                d.child(
                    div().id(ElementId::Name("header".into())).pt_3().px_3().gap_2().child(header),
                )
            })
            .child(
                div().id(ElementId::Name("content".into())).flex_1().min_h_0().child(
                    div().id(ElementId::Name("inner".into())).p_3().overflow_scroll().children(
                        self.content.into_iter().enumerate().map(|(ix, c)| {
                            div().id(ElementId::Integer(ix as u64)).mt_3().child(c.collapsed(self.collapsed))
                        }),
                    ),
                ),
            )
