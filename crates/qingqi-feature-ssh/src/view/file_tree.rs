...
fn render_toolbar(tree: &FileTreeViewModel) -> impl IntoElement {
    div()
        .h(px(36.0)).flex().items_center().px_2().gap(px(4.0))
...
fn render_tool_button(label: &'static str, id: &'static str) -> impl IntoElement {
    div()
        ...
        .text_size(px(11.0)).text_color(ui::text_secondary())
        .cursor_pointer()
    ...
    if count > 50 {
        ...
        uniform_list("ssh-file-list", count, move |range, _w, _cx| {
            entries[range].iter().map(|(name, is_dir, size_text, is_selected)| {
                div()
                    .h(px(28.0)).flex().items_center().px_2().text_size(px(12.0)).cursor_pointer()
                    .bg(if *is_selected { hsla(0.55, 0.3, 0.5, 0.15) } else { hsla(0.0, 0.0, 0.0, 0.0) })
                    .hover(|s| { s.bg(hsla(0.55, 0.1, 0.5, 0.1)) })
                    .child(div().flex().items_center().gap(px(6.0))
                        .child(if *is_dir { "📁" } else { "📄" }).child(name.clone()))
                    .child(div().flex_1())
                    .child(div().text_size(px(11.0)).text_color(ui::text_secondary()).child(size_text.clone()))
                    .into_any_element()
            }).collect::<Vec<_>>()
        ...
        div().flex_1().overflow_y_scrollbar()
            .children(tree.entries.iter().map(|e| {
                div()
                    ...
                    .hover(|s| { s.bg(hsla(0.55, 0.1, 0.5, 0.1)) })
                    .child(div().flex().items_center().gap(px(6.0))
                        .child(if e.is_dir { "📁" } else { "📄" }).child(e.name.clone()))
                    .child(div().flex_1())
                    .child(div().text_size(px(11.0)).text_color(ui::text_secondary()).child(e.size_text.clone()))
            }))
            .into_any_element()
    }
}
