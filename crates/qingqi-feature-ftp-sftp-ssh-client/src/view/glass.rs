...
pub fn bg(dark: bool) -> Hsla {
    if dark {
        theme::rgba_with_alpha(semantic().bg_surface, 0.78)
    } else {
        theme::rgba_with_alpha(theme::white(), 0.86)
    }
}
...
pub fn shadow() -> Vec<BoxShadow> {
    vec![
        BoxShadow {
            color: theme::rgba_with_alpha(semantic().shadow, 0.08),
            offset: point(px(0.0), px(8.0)),
            blur_radius: px(24.0),
            spread_radius: px(-8.0),
        },
        BoxShadow {
            color: theme::rgba_with_alpha(semantic().shadow, 0.04),
