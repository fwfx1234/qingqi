...
use crate::model::{
    AuthMethod, ProtocolLogEntry, ProtocolLogKind, RemoteFileItem, RemoteProfile, RemoteProtocol,
    join_remote_path,
};
...
pub trait RemoteBackend: Send {
    ...
    fn stop_port_forward(&self, local_port: u16) -> Result<()> {
        let _ = local_port;
        Err(anyhow!("端口转发仅支持 SSH/SFTP 连接"))
    }
}
...
impl ProtocolLogBuffer {
    ...
    fn push(&self, kind: ProtocolLogKind, text: impl Into<String>) {
        if let Ok(mut items) = self.items.lock() {
            ...
            if items.len() > 500 {
                let drain = items.len().saturating_sub(500);
                items.drain(0..drain);
            }
        }
    }
            ...
            .spawn(move || {
                ...
                while !reader_closed.load(Ordering::SeqCst) {
                    ...
                    if let Ok(mut lines) = reader_outputs.lock() {
                        ...
                        if lines.len() > 400 {
                            let drain = lines.len().saturating_sub(400);
                            lines.drain(0..drain);
                        }
                    }
...
impl SftpBackend {
    fn connect_via_jump_host(&mut self, timeout: Duration) -> Result<()> {
        ...
        target_sess.set_timeout(timeout.as_millis() as u32);
...
impl RemoteBackend for SftpBackend {
    fn connect(&mut self) -> Result<()> {
        ...
        if self.profile.jump_enabled && !self.profile.jump_host.trim().is_empty() {
    ...
    fn start_port_forward(
        ...
        tcp.set_nonblocking(false).context("设置阻塞模式失败")?;
        let mut buf = [0u8; 8192];
        let n = tcp.read(&mut buf).context("读取本地数据失败")?;
        ch.write_all(&buf[..n]).context("写入隧道失败")?;
        let n = ch.read(&mut buf).context("读取隧道失败")?;
        tcp.write_all(&buf[..n]).context("写入本地失败")?;
        Ok(())
    }

    fn stop_port_forward(&self, _local_port: u16) -> Result<()> {
        Ok(())
    }
}
...
impl RemoteBackend for FtpBackend {
    fn connect(&mut self) -> Result<()> {
        ...
        if self.profile.protocol == RemoteProtocol::Ftps {
            ...
            // TlsConnector<Stream=NoTlsStream>，而所有 TLS connector 均不满足此约束。
            // 解决方案：需要后续引入支持 TLS 的 FTP 库或自行实现 FTP+TLS。
