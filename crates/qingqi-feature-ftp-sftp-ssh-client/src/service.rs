...
use crate::{
    backend::{
        RemoteTerminal, create_backend, make_remote_version_hint, now_unix_secs, spawn_file_monitor,
    },
    manifest::PLUGIN_ID,
    model::{
...
impl FtpSftpSshService {
    ...
    pub fn open_text_file(self: &Arc<Self>, item: &RemoteFileItem) -> Result<()> {
        ...
        thread::spawn(move || {
            ...
            match result {
                Ok(()) => {
                    let _ = qingqi_platform::shell::open_path(Path::new(&local_path));
    ...
    pub(crate) fn open_store(&self) -> Result<RemoteProfileStore> {
        RemoteProfileStore::open(
            ...
            &qingqi_plugin::database::feature_database_key(PLUGIN_ID, "profiles"),
        )
    }
...
mod tests {
    ...
    use crate::model::{RemoteProtocol, RightPanelMode};
