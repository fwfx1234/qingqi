...
impl ImageCompressView {
    ...
    pub fn choose_images(&mut self) {
        match qingqi_platform::shell::choose_file("选择一张图片导入") {
    ...
    pub fn paste_from_clipboard(&mut self, cx: &App) {
        ...
        if let Some(clipboard_image) = qingqi_platform::clipboard::read_image(cx) {
            let ext = qingqi_platform::clipboard::image_format_extension(clipboard_image.format);
            match self
                .service
    ...
    pub fn choose_output_dir(&mut self) {
        match qingqi_platform::shell::choose_directory("选择压缩输出目录") {
            Ok(Some(path)) => {
    ...
    pub fn open_output_dir(&mut self) {
        ...
        thread::spawn(move || {
            ...
            let _ = qingqi_platform::shell::open_path(&dir);
        });
    ...
    pub fn reveal_entry(&mut self, index: usize) {
        ...
        match qingqi_platform::shell::open_path(&output_path) {
                ...
                .child(
                    div()
                        ...
                        .children(if is_success {
                            Some(
                                Button::new(("image-compress-copy", index)).label("复制").with_size(Size::Small)
                                    .on_click({
                                        let handle = handle.clone();
                                        move |_, window, cx| {
                                            handle.update(cx, |this, cx| {
                        ...
                        .children(if is_success {
                            Some(
                                Button::new(("image-compress-reveal", index)).label("定位").with_size(Size::Small)
                                    .on_click({
                                        let handle = handle.clone();
                                        move |_, window, cx| {
                                            handle.update(cx, |this, _cx| {
                        ...
                        .children(if is_success && !from_clipboard && has_source {
                            Some(
                                Button::new(("image-compress-overwrite", index)).label("覆盖").with_size(Size::Small)
                                    .on_click({
                                        let handle = handle.clone();
                                        move |_, window, cx| {
                                            handle.update(cx, |this, cx| {
                        ...
                        .children(if is_success {
                            Some(
                                Button::new(("image-compress-save-as", index)).label("另存").with_size(Size::Small)
                                    .on_click({
                                        let handle = handle.clone();
                                        move |_, window, cx| {
                                            handle.update(cx, |this, _cx| {
                        ...
                        .children(if is_failed {
                            Some(
                                Button::new(("image-compress-retry", index)).label("重试").with_size(Size::Small)
                                    .on_click({
                                        let handle = handle.clone();
                                        move |_, window, cx| {
                                            handle.update(cx, |this, cx| {
            ...
            div()
                ...
                .child(
                    div()
                        .flex_1()
                        .text_size(px(11.0))
                        .text_color(tokens(cx).muted_foreground)
                        .child(message),
                )
                .child(
                    div()
...
mod tests {
    use super::*;
    use crate::service::ImportPreview;
