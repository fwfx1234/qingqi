...
impl AppIndexService {
    ...
    pub fn with_events(
        ...
        Self::build(database, usage_store, Some(events))
    }
    ...
    pub fn record_launch(&self, path: &str) -> anyhow::Result<()> {
        let key = format!("app:{path}");
        if let Err(error) = self.usage_store.record_launch(&key) {
            tracing::warn!(path, error = %error, "app launch usage record failed");
        }
    ...
    fn refresh_index(&self) {
        ...
        self.publish_metadata_early(apps.clone());
    ...
    fn publish_metadata_early(&self, apps: Vec<AppEntry>) {
        {
            let mut state = lock_or_recover(&self.state, "app-index");
            state.apps = apps;
...
fn app_index_step_warn_threshold_ms(step: &str) -> u64 {
    match step {
        ...
        | "app index metadata scan"
        | "app index icon refresh"
        | "app index full cache save" => SLOW_SCAN_STEP_WARN_MS,
...
mod tests {
    ...
    fn test_store(name: &str) -> (AppIndexStore, CommandUsageStore) {
        ...
        database
            ...
            .unwrap();
        (
            AppIndexStore::new(Arc::clone(&database), "app-launcher/index"),
            CommandUsageStore::new(Arc::clone(&database), "command-usage"),
        )
    }

    #[test]
    fn publish_metadata_early_marks_icon_refresh_running() {
        // We don't need a real DB for this test — just check state transitions.
        let (store, usage_store) = test_store("early.db");
        ...
        service.publish_metadata_early(vec![AppEntry {
