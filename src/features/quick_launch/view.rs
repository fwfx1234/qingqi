...
use crate::{
    ...
    features::quick_launch::{
        model::{
            ActionKind, FeedbackMode, QuickAction, QuickActionDraft, QuickRun, RunStatus,
...
impl QuickLaunchView {
    ...
    fn pick_editor_target(&mut self, cx: &mut Context<Self>) {
        ...
        match platform::shell::choose_file(prompt) {
            Ok(Some(path)) => {
                if let Some(editor) = self.editor.as_mut() {
                    editor.target_input.update(cx, |input, input_cx| {
    ...
    fn pick_editor_cwd(&mut self, cx: &mut Context<Self>) {
        ...
        match platform::shell::choose_directory("选择工作目录") {
            Ok(Some(path)) => {
    ...
    fn copy_result_stdout(&mut self, cx: &mut Context<Self>) {
        ...
        if result.run.stdout.trim().is_empty() {
            ...
            platform::clipboard::write_text(cx, result.run.stdout.clone());
    ...
    fn copy_result_stderr(&mut self, cx: &mut Context<Self>) {
        ...
        if result.run.stderr.trim().is_empty() {
            ...
            platform::clipboard::write_text(cx, result.run.stderr.clone());
    ...
    fn open_pending(
        &mut self,
        action: QuickAction,
        specs: Vec<crate::features::quick_launch::parameters::ParameterSpec>,
        cx: &mut Context<Self>,
    ) {
        let fields = specs
            .into_iter()
...
fn empty_state(has_query: bool, dark: bool) -> impl IntoElement {
...
fn editor_field(label: &'static str, input: Entity<TextInput>, dark: bool) -> impl IntoElement {
    div()
        .flex()
        .flex_col()
        .gap_1()
...
fn parameter_row(field: PendingParameterField, index: usize, dark: bool) -> impl IntoElement {
    div()
...
fn action_menu_separator(dark: bool) -> impl IntoElement {
    div()
        .w_full()
        .h(px(1.0))
        .my(px(2.0))
...
fn kind_chip(label: String, dark: bool) -> impl IntoElement {
    div()
        .h(px(20.0))
        .px(px(8.0))
        .rounded(px(4.0))
...
fn subtle_chip(label: String, dark: bool) -> impl IntoElement {
    div()
        .h(px(20.0))
        .px(px(8.0))
        .rounded(px(4.0))
...
fn icon_for_action(action: &QuickAction) -> String {
    ...
    match action.kind {
        crate::features::quick_launch::model::ActionKind::Script => String::from("⌘"),
        crate::features::quick_launch::model::ActionKind::OpenPath => String::from("📁"),
        crate::features::quick_launch::model::ActionKind::OpenUrl => String::from("🌐"),
    }
}
