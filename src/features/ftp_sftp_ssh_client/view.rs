...
impl FtpSftpSshPanel {
    ...
    fn reopen_local_draft(&mut self, draft: &RemoteEditDraft) {
        ...
        let _ = platform::shell::open_path(Path::new(&draft.local_cache_path));
    }
}
...
impl RenderOnce for FtpSftpSshElement {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let accent = theme::accent_color(theme::ThemeAccent::Cyan);
        let accent_soft = if dark {
            theme::accent_soft_dark(theme::ThemeAccent::Cyan)
        } else {
            theme::accent_soft(theme::ThemeAccent::Cyan)
        };
...
fn search_input_shell(input: Option<Entity<TextInput>>, dark: bool) -> impl IntoElement {
    div()
        .rounded(px(8.0))
        .border_1()
        .border_color(ui::border_light())
