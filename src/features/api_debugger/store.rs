...
use crate::features::api_debugger::model::{
...
impl ApiDebuggerStore {
    pub fn open(path: &Path) -> Result<Self> {
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)
                .with_context(|| format!("无法创建 API 调试器目录 {}", parent.display()))?;
        }
    ...
    fn list_env_headers_locked(&self, conn: &Connection, env_id: &str) -> Result<Vec<EnvHeader>> {
        let mut stmt = conn.prepare(
...
use crate::features::api_debugger::model::{ApiEnvironment, ApiGroup};
...
mod tests {
    ...
    fn open_store() -> ApiDebuggerStore {
        ApiDebuggerStore::open(&temp_db()).unwrap()
    }
