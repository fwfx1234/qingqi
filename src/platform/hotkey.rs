...
pub struct HotkeyRegistrationResult {
    ...
    pub low_level_fallbacks: Vec<(String, crate::platform::low_level_hook::LowLevelEntry)>,
}
                ...
                Err(error) => {
                    #[cfg(target_os = "windows")]
                    {
                        ...
                        if let Some(entry) = try_as_low_level_entry(shortcut_id, hotkey) {
                            ...
                            result.low_level_fallbacks.push((shortcut_id.clone(), entry));
                            continue;
                        }
                    }
                    let _ = shortcut_id;
...
fn try_as_low_level_entry(
    shortcut_id: &str,
    hotkey: &HotKey,
) -> Option<crate::platform::low_level_hook::LowLevelEntry> {
    use crate::platform::low_level_hook::parse_low_level_entry;
